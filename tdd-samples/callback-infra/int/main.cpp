...
TEST_CASE_FIXTURE(WorkerShould, "schedule a callback with local lambda" * doctest::timeout(2.0)) {
...
TEST_CASE_FIXTURE(WorkerShould,
                  "schedule a callback with free function/ class member func" * doctest::timeout(2.0)) {
...
TEST_CASE_FIXTURE(WorkerShould, "invoke Callback repeatedly until cancelled" * doctest::timeout(3.0)) {
  auto worker = WorkerImpl{};
  unsigned called{0};
  unsigned multiple{3};
  auto fn = [&called]() {
